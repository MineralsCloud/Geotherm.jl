...
using Geometry: Point, Rectangle, Manifold
...
function bind(p0::Point{T}, ts::Vector{T}, ps::Vector{T}, h=0.01, n=1000) where T <: Real
    ...
    for k in 1:(n - 1)
        ...
        p_next = runge_kutta_iter(trace[k], bilinear_interpolate(
            Manifold(Point(ts[i], ps[j]), adiabat[i, j]),
            Manifold(Point(ts[i+1], ps[j]), adiabat[i+1, j]),
            Manifold(Point(ts[i], ps[j+1]), adiabat[i, j+1]),
            Manifold(Point(ts[i+1], ps[j+1]), adiabat[i+1, j+1])
        ), h)
        push!(trace, p_next)
    end
    trace
