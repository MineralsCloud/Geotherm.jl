...
!!! info
    The style guide is always a work in progress, and not all Geotherm code
    follows the rules. When modifying Geotherm, please fix the style violations
...
Geotherm uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/Geotherm.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the Geotherm directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to Geotherm have
